...
public class Main extends PApplet
{
    ...
    public void drawPath()
    {
        ...
        Point myPoint = view.viewportToWorld(WorldView.viewport, x, y);

        if (world.isOccupied(myPoint))
        {
            try
            {
                MobileAnimatedActor myActor = (MobileAnimatedActor) world.getTileOccupant(myPoint);
                ArrayList<Point> path = myActor.get_path();
                for (int i = 0; i < path.size() - 1; i++)
                {
                    Point p2 = view.worldToViewport(WorldView.viewport, path.get(i).x, path.get(i).y);
                    image(loadImage("star.png"), (p2.x * TILE_WIDTH) + (TILE_WIDTH / 8),
                            (p2.y * TILE_HEIGHT) + (TILE_HEIGHT / 8));
                }
            }
